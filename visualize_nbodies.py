...
def update(frame_idx):
    ...
    # 2. Update Heatmap (Weighted by Mass)
    hist, _, _ = np.histogram2d(x_data, y_data, bins=GRID_RES,
                                range=[[-LIMIT, LIMIT], [-LIMIT, LIMIT]],
                                weights=masses)

    img.set_data(hist.T)

    fig.suptitle(f"Frame {frame_idx}/{len(frames)}", fontsize=16)
    return scatter, img

# Blit=False is safer for complex multi-axis animations on some backends
